...
const OUTPUT_TO_BASE_FIELD: Partial<Record<keyof ContentItem, string>> = {
  ...
  recommendationTrack: CONTENT.recommendationTrack,
  timeToValue: CONTENT.timeToValue,
  adoptionLevel: CONTENT.adoptionLevel,
  networkRequirement: CONTENT.networkRequirement,
...
function mapContentRecord(record: RawFeishuRecord, copyBlocks: CopyBlock[]): ContentItem {
  ...
  const candidate = {
    ...
    recommendationTrack: requireString(record, CONTENT.recommendationTrack),
    timeToValue: requireString(record, CONTENT.timeToValue),
    adoptionLevel: requireString(record, CONTENT.adoptionLevel),
    networkRequirement: requireString(record, CONTENT.networkRequirement),
    takeaway: requireString(record, CONTENT.takeaway),
